...
export const TeamCreate: React.FC = () => {
  ...
  const fetchCurrentUser = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Set firm_id in form data when user is loaded (note: API returns firmId in camelCase)
        setFormData(prev => ({ ...prev, firm_id: data.user.firmId }));
      }
    } catch (error) {
  ...
  const createTeam = async () => {
    ...
    if (!formData.firm_id) {
      alert('Unable to determine firm. Please try refreshing the page.');
      return;
    }
  ...
  const addMember = (userId: string) => {
    if (!formData.initial_member_ids.includes(userId)) {
      setFormData({
        ...formData,
        initial_member_ids: [...formData.initial_member_ids, userId]
      });
    }
  };
  ...
  const removeMember = (userId: string) => {
    setFormData({
      ...formData,
      initial_member_ids: formData.initial_member_ids.filter(id => id !== userId)
    });
  };

  const selectedUsers = availableUsers.filter(user => 
    formData.initial_member_ids.includes(user.id)
  );
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-100">Create New Team</h1>
            <p className="text-slate-400">Create a team and assign members to collaborate on matters</p>
          </div>
        </div>
          ...
          <Card>
            <CardContent className="pt-6 space-y-4">
              ...
              {selectedUsers.length > 0 && (
                <div className="space-y-2">
                  <Label>Selected Team Members ({selectedUsers.length})</Label>
